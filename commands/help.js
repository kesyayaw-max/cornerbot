...
const HELP_PANELS = {
  home: {
    ...
    fields: [
      ...
      { name: '🛡️ Admin', value: 'Event, broadcast, admincash, resetvoice (khusus admin server).', inline: true },
  ...
  admin: {
    ...
    color: COLORS.admin,
    description: 'Command khusus admin server / OWNER_ID buat kelola Cosmic Corner.',
    commands: [
      ['event add <judul> | <YYYY-MM-DD HH:mm> | <deskripsi> | <gambar opsional>', 'Tambah event baru, otomatis tampil di website.'],
      ['event list', 'Lihat semua event yang tercatat beserta ID-nya.'],
      ['event remove <id>', 'Hapus event berdasarkan ID.'],
      ['broadcast <#channel> <pesan>', 'Kirim pengumuman ke channel tertentu.'],
      ['admincash <set/add/remove> @user <jumlah>', 'Atur coin milik member.'],
