...
function renderStats(stats){
  const map = {
    ...
    statBoosts: stats ? num(stats.boostCount) : '-',
  };
...
function renderMemberOfWeek(data){
  const section = document.getElementById('motw');
  ...
  const avatarHtml = m.avatar
    ? `<img class="motw-avatar" src="${m.avatar}" alt="${escapeHtml(m.name)}" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'"/>`
    : `<div class="motw-avatar motw-avatar-fallback">${escapeHtml((m.name||'?').charAt(0).toUpperCase())}</div>`;

  card.innerHTML = `
    ${avatarHtml}
    <div class="motw-info">
      <div class="motw-name">${escapeHtml(m.name)}</div>
      <div class="motw-highlight">✨ ${escapeHtml(m.highlight)}</div>
      <div class="motw-stats">
        <span>⭐ Lv.${num(m.level)}</span>
        <span>💰 ${num(m.coin)}</span>
        <span>🎧 ${escapeHtml(m.voiceTime)}</span>
        <span>🐾 ${num(m.petCount)}</span>
        <span>🏆 ${num(m.wins)}</span>
      </div>
    </div>
  `;
...
function initShopPage(){
  ...
  function itemCard(item){
    return `
      <div class="shop-card">
        ...
        <div class="shop-price">${num(item.price)} <span>coin</span></div>
        <code class="shop-cmd">cc buy ${item.key}</code>
      </div>
    `;
  }
  ...
  function draw(items){
    if (!items.length) {
      content.innerHTML = `<div class="state-box"><span class="emoji">🛒</span>Belum ada item di shop.</div>`;
      return;
    }
    content.innerHTML = `<div class="shop-grid">${items.map(itemCard).join('')}</div>`;
  }
...
/* ---------- Scroll-reveal for cards/sections (runs on every page) ---------- */
document.addEventListener('DOMContentLoaded', () => {
  const targets = document.querySelectorAll(
    '.story-grid, .feature-grid, .stats-bar, .about-section, .features-section, .motw-section'
  );
